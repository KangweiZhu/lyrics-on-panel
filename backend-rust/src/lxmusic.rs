...
impl LxMusicManager {
    ...
    pub async fn current_lyric(
        ...
        let lyric = state.read().await.current_lyric(track_title, position_us);
        lyric
    }
    ...
    fn remove_oldest_subscription(subscriptions: &mut HashMap<u16, Subscription>) {
        ...
        if let Some(port) = oldest_port {
            if let Some(subscription) = subscriptions.remove(&port) {
                subscription.abort_handle.abort();
            }
        }
    }
...
impl PlaybackStatus {
    fn parse(value: &str) -> Option<Self> {
        match value {
            ...
            "stoped" | "stopped" => Some(Self::Stopped),
            _ => None,
        }
    }
}
...
mod tests {
    ...
    use super::{LxMusicManager, PlayerStatus, SseEvent, SseParser, MAX_SUBSCRIPTIONS};
