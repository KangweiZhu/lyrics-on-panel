...
fn select_player<'a>(
    ...
    if let Some(requested) = requested.filter(|name| !name.is_empty()) {
        let selected = if matches!(
            requested,
            "lx-music-desktop" | "org.mpris.MediaPlayer2.lx-music-desktop"
        ) {
            players
                .iter()
...
mod tests {
    ...
    use tokio_tungstenite::{
        connect_async,
        tungstenite::{protocol::frame::coding::CloseCode, Message as TungsteniteMessage},
    };

    use crate::model::{PlaybackStatus, PlayerState, Track};

    use super::{
        healthcheck, json_message_bytes, poll_interval, select_control_player, select_player,
        unknown_endpoint, ControlRequest, PollRequest, ResponseLimiter,
    };
    ...
    fn strict_and_alias_selection_do_not_fallback() {
        ...
        assert_eq!(
            select_player(
                &players,
                Some("org.mpris.MediaPlayer2.lx-music-desktop"),
                &mut current,
            )
            .unwrap()
            .bus_name,
